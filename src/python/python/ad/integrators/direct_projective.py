...
class DirectProjectiveIntegrator(PSIntegrator):
    ...
    def sample_radiance_difference(self, scene, ss, curr_depth, sampler, wavelengths, active):
        if curr_depth == 1:
            ...
            # contribute here. We reconstruct the foreground surface interaction
            # from the silhouette parameterization. ``eval_parameterization``
            # maps ``ss.uv`` to a surface interaction consistently with each
            # shape's UV convention. Feeding ``ss.uv`` into ``pi.prim_uv``
            # directly would be wrong for shapes whose texture UV differs from
            # the primitive UV (e.g. curves, where ``ss.uv.x`` is the azimuth but
            # ``prim_uv.x`` is the segment parameter), placing the foreground
            # point -- and hence its normal/radiance -- at the wrong location.
            ...
            # ----------- Estimate the radiance of the foreground -----------
            # Reconstruct the foreground interaction from the silhouette
            # parameterization (see the note in the ``curr_depth == 1`` branch).
            si_fg = ss.shape.eval_parameterization(
                ss.uv, mi.RayFlags.All, active)
            si_fg.wavelengths = wavelengths
